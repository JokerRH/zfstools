/*
	Modified from the rijndael reference code. See original disclaimer below.
*/
...
static void AddRoundKey( word8 a[ 4 ][ MAXBC ], word8 rk[ 4 ][ MAXBC ] )
{
...
static void SubBytes( word8 a[ 4 ][ MAXBC ], word8 box[ 256 ] )
{
...
static void ShiftRows( word8 a[ 4 ][ MAXBC ], word8 d )
{
...
static void MixColumns( word8 a[ 4 ][ MAXBC ] )
{
...
static void InvMixColumns( word8 a[ 4 ][ MAXBC ] )
{
	/* Mix the four bytes of every column in a linear way
...
static int KeyExpansion( word8 k[ 4 ][ MAXKC ], word8 W[ MAXROUNDS + 1 ][ 4 ][ MAXBC ] )
{
...
static int Encrypt( word8 a[ 4 ][ MAXBC ], word8 rk[ MAXROUNDS + 1 ][ 4 ][ MAXBC ] )
{
...
static int Decrypt( word8 a[ 4 ][ MAXBC ], word8 rk[ MAXROUNDS + 1 ][ 4 ][ MAXBC ] )
{
	int r;
	/* To decrypt:
...
static void _Encrypt( const word8 *aIn, word8 *aOut, const word8 *aKey )
{
	word8 a[ 4 ][ MAXBC ];
	word8 rk[ MAXROUNDS + 1 ][ 4 ][ MAXBC ];
	word8 sk[ 4 ][ MAXKC ];

	for( unsigned col = 0; col < BC; ++col )
		for( unsigned row = 0; row < 4; ++row )
			a[ row ][ col ] = aIn[ col * 4 + row ];

	for( unsigned col = 0; col < KC; ++col )
		for( unsigned row = 0; row < 4; ++row )
			sk[ row ][ col ] = aKey[ col * 4 + row ];
	...
	for( unsigned col = 0; col < BC; ++col )
		for( unsigned row = 0; row < 4; ++row )
			aOut[ col * 4 + row ] = a[ row ][ col ];
}

static void _Decrypt( const word8 *aIn, word8 *aOut, const word8 *aKey )
{
	word8 a[ 4 ][ MAXBC ];
	word8 rk[ MAXROUNDS + 1 ][ 4 ][ MAXBC ];
	word8 sk[ 4 ][ MAXKC ];
...
void Encrypt128_128( const word8 in[ 16 ], word8 out[ 16 ], const word8 key[ 16 ] )
{
	KC = 4;
	BC = 4;
